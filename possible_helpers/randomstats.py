def jGvsG_stats(jG):
	...
	if jG.G.data.babyRNA.SNP_to_genomic_region.has_key(x):
    ...
    jGBL = map(len,map(lambda x: jG.components[x], jG.comp_mins))
    GBL = map(len,map(lambda x: G.components[x], G.comp_mins))
    bjG = sum(map(len,map(lambda x: jG.components[x], jG.comp_mins)))/float(len(jG.comp_mins))
    bG = sum(map(len,map(lambda x: G.components[x], G.comp_mins)))/float(len(G.comp_mins))
