...
def make_solution(X,G,outputname,name):
    ...
    for start in G.comp_mins:
        ...
        f.write('BLOCK' +
                ' Start: '+ str(start+1) +
                ' Len: ' + str(len(G.components[start]))+
                ' Phased: ' + str(len(G.components[start]))+
                ' Span: ' + str(e_pos-s_pos)+
                ' MEC: ' + str(comp_MEC(start,X,G))+
                ' Reads: '+ str(len(G.comp_reads[start])) 
                +' \n')
            
        for i in G.components[start]:
            s = ''
            s = s + str(i+1) + t + str(X[start][0][i]) + t + str(X[start][1][i]) + t + str(G.nodes[i].chrom) + t + str(G.nodes[i].position) + t
            s += '\n'
            f.write(s)
        f.write('*****\n')
    f.close()
...
def make_solution_RNA(X,RD,outputname,name):
    ...
    for start in X:
        ...
        f.write('BLOCK' +
                ' Start: '+ str(start+1) +
                ' Len: ' + str(len(X[start][0]))+
                ' Phased: ' + str(len(X[start][0]))+
                ' Span: ' + str(e_pos-s_pos)+
                ' MEC: ' + '##' +
                ' Reads: '+ '##'
                +' \n')
        ...
        for i in X[start][0]:
            s = ''
            s = s + str(i+1) + t + str(X[start][0][i]) + t + str(X[start][1][i]) + t + str(RD.nodes[i].chrom) + t +  str(RD.nodes[i].position) + t
            s += '\n'
            f.write(s)
        f.write('*****\n')
    f.close()
...
def make_solution_simple(X,outputname,name, vChroms, vPositions):
    #writes phasing solution to outputname
    t = '\t'
    f = open(outputname+'/'+name,'w')
    for start in X:
        ...
        for i in X[start][0]:
            s = ''
            s = s + str(i+1) + t + str(X[start][0][i]) + t + str(X[start][1][i]) + t + str(vChroms[i]) + t + str(vPositions[i]) + t
            s += '\n'
            f.write(s)
        f.write('*****\n')
    f.close()
