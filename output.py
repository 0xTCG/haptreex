...
def make_solution(V: VCF, G: Graph, phases: Dict[int, Phase], path: str):
    ...
    with open(path, "w") as f:
        for root, comp in sorted(G.components.items()):
            span = V.snps[comp.snps[-1]].pos - V.snps[comp.snps[0]].pos
            ...
            f.write(
                ...
                f"Start: {root} "
                f"Len: {len(comp.snps)} "
                f"Phased: {len(comp.snps)} "
                f"Span: {span} "
                f"MEC: {mec} "
                f"Reads: {reads}\n"
            )
            haps = sorted(phases[root].haplotypes, key=lambda d: d[comp.snps[0]])
            for snp in comp.snps:
                f.write(f"{snp}\t")
